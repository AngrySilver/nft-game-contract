...
module.exports = async deployer => {  
  ...
  let _lxr = await LXR.deployed();
  
  await deployer.deploy(LXRNFT);
  let _lxrnft = await LXRNFT.deployed();

  await deployer.deploy(LXRGAME, _lxr.address, _lxrnft.address);
  let _lxrgame = await LXRGAME.deployed();
  
  // set baseURI for nft contract
  // set whitelist for nft contract
  // transfer reward tokens from LXR contract to Game contract
  await _lxrnft.setBaseURI(baseURI);

  await _lxrnft.addWhitelist(testwhitelist);
